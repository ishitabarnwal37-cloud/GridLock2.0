...
def load_logistics_data():
    try:
        df = pd.read_csv("Cleaned_Data_V2.csv")
        # Reverse Trigonometry: Convert Sine/Cosine back into human-readable hours/days
...
# ==========================================
st.sidebar.image("https://upload.wikimedia.org/wikipedia/en/thumb/0/08/Flipkart_logo.png/800px-Flipkart_logo.png", width=150)
...
# ==========================================
# PAGE 3: FLIPKART LOGISTICS INTELLIGENCE
# ==========================================
    ...
    # 1. Define Warehouse and Destination Zones
        warehouse_coords = [13.0645, 77.4585] # Approx coords for your provided address
    
        destination = st.selectbox("Select Delivery Destination Zone:", 
                               options=["Koramangala", "Bellandur", "HSR Layout", "Indiranagar", "Whitefield"])
    
        if st.button("Calculate Route Safety"):
        # 2. Logic to filter data for that specific area
        # (Assuming you add a column for neighborhood or filter by lat/lon distance)
            st.write(f"Analyzing historical congestion between Warehouse and {destination}...")
        
        # Simple Logic: Check if the number of violations in the chosen area 
        # is higher than the average congestion of the city for that hour
